...
from pyencrypt.license import (FIELDS, check_license, generate_license_file, get_host_ipv4, get_mac_address)
...
class TestGenerateLicense:
    ...
    def test_check_license_expired(self, key, after, before, tmp_path):
        license_file_path = generate_license_file(key, path=tmp_path, after=after, before=before)
        with pytest.raises(Exception) as excinfo:
            check_license(license_file_path, key)
        assert str(excinfo.value) == 'License expired.'

    @pytest.mark.parametrize('key,mac_addr', [
